...
```bash
python manage.py create_load_testing_fixture --confirm
```

Note: The command as implemented does not require `--confirm`. Use:
...
-   No sequence reset is performed after loading fixtures.
-   This is fine because the load test DB is intended to be single-use and the
    load test flow should not create additional rows relying on sequences.
...
-   Forces Turnstile to always-pass test keys by default (env vars can override)
-   Uses console email backend
-   Uses dev/staging buckets for safety
...
All authenticated users are allowed to review (no special permissions).
Anonymous users cannot use reviewer flows.
...
-   Typically dev/staging prepared from a refreshed copy of production
-   Ensure you are on VPN and can reach the target host
...
-   Global abort "no work":
    ...
    -   Asset pages do not include the transcription form (UI mismatch)
-   Lots of 403s:
...
To drop after loading (validation-only), you can use:
...
This is not the normal load test flow since it drops the DB immediately after
loading.

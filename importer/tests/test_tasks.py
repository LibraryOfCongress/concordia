...
class DownloadWriteCollectionItemAssetsTest(TestCase):
    ...
    def test_download_write_collection_item_asstes(self, mock_get, mock_save):
        ...
        collection = {"collection_name": self.name, "collection_slug": slugify(self.name), "collection_task_id": "123",
                      "subcollection_name": self.project, "subcollection_slug": slugify(self.project)}
        ...
        mock_get.side_effect = [mock_resp_page, mock_page1_result, mock_page2_result, mock_resp_item_urls]
        mock_save.return_value = None

        # Act
        download_write_collection_item_assets(self.name, self.project, self.url)

        ctd = CollectionTaskDetails.objects.get(collection_slug=self.name, subcollection_slug=self.project)
    ...
    @patch('importer.tasks.get_save_item_assets')
    @patch('importer.tasks.requests.get')  # Mock 'requests' module 'get' method.
    def test_download_write_collection_item_asstes_no_db_entry(self, mock_get, mock_save):
        """

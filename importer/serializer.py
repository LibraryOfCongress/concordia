...
class CreateCollection(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if create_type not in create_types:
            raise serializers.ValidationError("The url not belongs to collections or item")
        if create_type == create_types[0]:
            try:
                CollectionTaskDetails.objects.get(collection_slug=slugify(data['name']),
                                              subcollection_slug=slugify(data['project']))
            except CollectionTaskDetails.DoesNotExist:
                pass
            else:
                raise serializers.ValidationError("collection and project already exist")
